...
Object.keys(object).map((key) => {
    const objects = {};
    const iterator = object[key].values();
    const obj = {};
    for (const value of iterator) {
        final[key] = {
            ...final[key],
            [value.subcounty[0] +
                value.subcounty.slice(1, value.subcounty.length).toLowerCase()]: {
                total: Number(value.total),
                male: Number(value.male),
                female: Number(value.female),
                intersex: Number(value.intersex),
                popDensity: {
                    landArea: Number(value.areaSqKm),
                    density: Number(value.popDensity),
                },
            },
        };
    }
});
fs_1.default.writeFileSync("./data.ts", JSON.stringify(final));
